...
.app-root {
  ...
  --glow-strong: rgba(0, 212, 255, 0.65);
...
.app-root.theme-light {
  ...
  --shadow-card: 0 4px 20px rgba(15, 23, 42, 0.05), 0 0 0 1px rgba(8, 145, 178, 0.06);
  --shadow-hover: 0 8px 28px rgba(8, 145, 178, 0.1);
  --shadow-purple: 0 8px 28px rgba(124, 58, 237, 0.08);
...
.app-root { width: 100%; height: 100vh; position: relative; overflow: hidden; background: var(--bg); color: var(--text); font-family: 'Inter', 'PingFang SC', 'Microsoft YaHei', system-ui, sans-serif; display: flex; flex-direction: column; }
...
.app-root.theme-light .title-text {
  background: linear-gradient(180deg, #1e293b 0%, #0891b2 50%, #7c3aed 100%);
...
.app-root.theme-light .tech-tab.active { box-shadow: 0 4px 16px rgba(124, 58, 237, 0.25); }
...
.bg-glow-1 { width: 700px; height: 700px; background: #00d4ff; top: -200px; left: -150px; opacity: 0.25; }
.bg-glow-2 { width: 800px; height: 800px; background: #a855f7; bottom: -250px; right: -150px; opacity: 0.2; animation-delay: -6s; }
...
.scan-line {
  ...
  box-shadow: 0 0 20px #00d4ff, 0 0 40px #00d4ff, 0 0 60px rgba(168, 85, 247, 0.4);
...
.tech-topbar {
  ...
  padding: 4px 24px;
  background: linear-gradient(90deg, transparent, rgba(0,212,255,0.06), rgba(168,85,247,0.04), transparent);
  border-bottom: 1px solid var(--border-soft);
}
.tech-corner { width: 26px; height: 26px; border: 1.5px solid var(--primary); position: relative; box-shadow: 0 0 10px var(--glow), 0 0 20px var(--glow-soft); }
.tech-corner-l { border-right: none; border-bottom: none; }
.tech-corner-r { border-left: none; border-bottom: none; }
.tc-dot { position: absolute; width: 6px; height: 6px; background: var(--primary); border-radius: 50%; box-shadow: 0 0 10px var(--primary), 0 0 20px var(--glow); animation: dot-blink 1.5s ease-in-out infinite; }
...
.line-pulse { position: absolute; top: 50%; width: 70px; height: 2px; background: linear-gradient(90deg, var(--primary), var(--secondary)); transform: translateY(-50%); box-shadow: 0 0 12px var(--primary); animation: line-move 3s ease-in-out infinite; }
...
@keyframes line-move { 0% { transform: translateY(-50%) translateX(0); opacity: 0; } 30% { opacity: 1; } 100% { transform: translateY(-50%) translateX(200px); opacity: 0; } }
...
.title-bracket { color: var(--secondary); font-weight: 700; text-shadow: 0 0 16px var(--secondary-glow), 0 0 30px var(--glow-purple); }
.title-text {
  background: linear-gradient(90deg, #ffffff 0%, #00d4ff 40%, #a855f7 80%, #ffffff 100%);
  background-size: 200% 100%;
  -webkit-background-clip: text; -webkit-text-fill-color: transparent;
  background-clip: text;
  filter: drop-shadow(0 0 14px var(--glow)) drop-shadow(0 0 28px var(--glow-soft));
  margin: 0 14px;
...
@keyframes shine-move { 0%, 100% { left: -30%; } 50% { left: 120%; } }
...
@keyframes title-underline-flow { 0% { background-position: -200% 0; } 100% { background-position: 200% 0; } }
...
.tc-digit { font-size: 20px; font-weight: 700; color: var(--primary); font-family: 'Consolas', 'Monaco', 'Courier New', monospace; text-shadow: 0 0 14px var(--glow), 0 0 6px var(--primary); letter-spacing: 2px; }
...
.tech-tab.active {
  ...
  box-shadow: 0 0 24px var(--glow), 0 0 36px var(--glow-purple);
...
@keyframes tab-border-move { 0% { background-position: -200% 0; } 100% { background-position: 200% 0; } }
.tech-tab-glow { position: absolute; bottom: 0; left: 50%; width: 60%; height: 1px; background: var(--primary); transform: translateX(-50%); opacity: 0; transition: opacity 0.3s; }
...
.left-col, .right-col, .center-col { display: flex; flex-direction: column; gap: 10px; min-height: 0; height: 100%; }
.center-col { gap: 10px; }
...
.bc-item:hover { border-color: var(--primary); box-shadow: 0 0 10px var(--glow-soft); }
...
.btn-action:hover { filter: brightness(1.1); box-shadow: 0 0 12px var(--glow); }
...
.tech-panel::before { top: -1px; left: -1px; border-right: none; border-bottom: none; animation: corner-pulse 3s ease-in-out infinite; }
.tech-panel::after { bottom: -1px; right: -1px; border-left: none; border-top: none; animation: corner-pulse 3s ease-in-out infinite -1.5s; }
@keyframes corner-pulse {
  0%, 100% { box-shadow: 0 0 10px var(--glow); }
  50% { box-shadow: 0 0 20px var(--glow), 0 0 30px var(--glow-soft); }
}
.tech-panel:hover { border-color: var(--border-hover); box-shadow: var(--shadow-hover), var(--shadow-purple); transform: translateY(-1px); }
.tech-panel-header {
...
.panel-bullet { position: relative; width: 8px; height: 8px; background: var(--primary); box-shadow: 0 0 10px var(--primary), 0 0 20px var(--glow); flex-shrink: 0; }
.panel-bullet::after { content: ''; position: absolute; inset: -4px; border: 1px solid var(--primary); border-radius: 50%; opacity: 0.6; animation: bullet-ring 2s ease-out infinite; }
@keyframes bullet-ring { 0% { transform: scale(0.5); opacity: 1; } 100% { transform: scale(2.5); opacity: 0; } }
.panel-title { font-size: 14px; font-weight: 600; color: var(--text); letter-spacing: 0.5px; flex: 1; text-shadow: 0 0 8px var(--glow-soft); }
.panel-tag { font-size: 10px; color: var(--primary); padding: 2px 10px; border: 1px solid var(--border); background: var(--primary-soft); letter-spacing: 0.5px; border-radius: 3px; box-shadow: 0 0 8px var(--glow-soft); }
.tech-panel-body { flex: 1; min-height: 120px; position: relative; }

.school-info { display: flex; align-items: center; gap: 12px; padding: 8px; }
.info-avatar {
...
.info-name { font-size: 15px; font-weight: 700; color: var(--text); margin-bottom: 2px; }
...
.teacher-item { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 8px; background: var(--bg-soft); border: 1px solid var(--border-soft); border-radius: 6px; }
.teacher-avatar { font-size: 20px; }
.teacher-name { font-size: 11px; color: var(--text-dim); }
.teacher-count { font-size: 14px; font-weight: 700; color: var(--primary); font-family: 'Consolas', monospace; }

.kpi-grid { display: flex; flex-direction: column; gap: 10px; height: 100%; min-height: 0; overflow: hidden; }
.kpi-row-large { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
.kpi-card-lg {
  position: relative;
  padding: 10px 12px;
...
.kpi-card-lg:hover { transform: translateY(-2px); border-color: var(--border-purple); box-shadow: 0 0 24px var(--glow-purple); }
.kpi-card-deco { position: absolute; top: 0; left: 0; right: 0; height: 2px; background: var(--grad-main); animation: deco-scan 3s linear infinite; box-shadow: 0 0 10px var(--glow); }
@keyframes deco-scan { 0% { transform: translateX(-100%); } 100% { transform: translateX(100%); } }
.kpi-card-hex { position: absolute; right: -8px; top: 50%; transform: translateY(-50%); width: 50px; height: 50px; opacity: 0.5; }
.kpi-card-hex span { position: absolute; inset: 0; border: 1.5px solid var(--primary); clip-path: polygon(50% 0, 100% 25%, 100% 75%, 50% 100%, 0 75%, 0 25%); animation: hex-rotate 10s linear infinite; }
...
@keyframes hex-rotate { 0% { transform: rotate(0deg) scale(1); } 100% { transform: rotate(360deg) scale(1); } }
.kpi-lg-label { font-size: 11px; color: var(--text-dim); letter-spacing: 0.5px; z-index: 1; }
.kpi-lg-value { font-size: 26px; font-weight: 700; background: var(--grad-main); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; font-family: 'Consolas', 'Monaco', monospace; filter: drop-shadow(0 0 10px var(--glow)); display: flex; align-items: baseline; gap: 2px; line-height: 1.1; z-index: 1; }
.kpi-lg-value .unit { font-size: 13px; color: var(--text-dimmer); -webkit-text-fill-color: var(--text-dimmer); }
.kpi-lg-trend { font-size: 10px; padding: 2px 8px; border-radius: 3px; z-index: 1; display: flex; align-items: center; gap: 3px; }
.kpi-lg-trend.up { color: var(--danger); background: rgba(244, 63, 94, 0.15); border: 1px solid rgba(244, 63, 94, 0.4); }
.kpi-lg-trend.down { color: var(--success); background: rgba(16, 185, 129, 0.15); border: 1px solid rgba(16, 185, 129, 0.4); }

.gender-item { display: flex; align-items: center; gap: 10px; padding: 8px 10px; background: linear-gradient(90deg, var(--bg-soft), transparent); border: 1px solid var(--border-soft); border-radius: 8px; transition: all 0.3s; position: relative; overflow: hidden; }
.gender-item::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 3px; }
.gender-item.male::before { background: var(--grad-cyan); }
.gender-item.female::before { background: var(--grad-purple); }
.gender-item:hover { border-color: var(--border); background: var(--primary-soft); transform: translateX(2px); }
.gender-icon { width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; border-radius: 50%; font-size: 16px; font-weight: bold; }
.gender-item.male .gender-icon { background: var(--primary-soft); color: var(--primary); }
.gender-item.female .gender-icon { background: var(--secondary-soft); color: var(--secondary); }
.gender-info { flex: 1; min-width: 0; }
.gender-label { display: block; font-size: 10px; color: var(--text-dim); margin-bottom: 2px; }
.gender-value { display: block; font-size: 16px; font-weight: 700; font-family: 'Consolas', 'Monaco', monospace; }
.gender-item.male .gender-value { color: var(--primary); }
.gender-item.female .gender-value { color: var(--secondary); }
.gender-bar { flex: 1; height: 4px; background: rgba(0, 212, 255, 0.08); border-radius: 2px; overflow: hidden; min-width: 40px; }
.gender-bar-fill { height: 100%; border-radius: 2px; transition: width 0.5s ease; }
.gender-item.male .gender-bar-fill { background: var(--grad-cyan); }
.gender-item.female .gender-bar-fill { background: var(--grad-purple); }

.kpi-row-small { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; }
.kpi-card-sm { position: relative; padding: 8px 6px; background: linear-gradient(135deg, var(--bg-soft), transparent); border: 1px solid var(--border-soft); border-radius: 8px; text-align: center; transition: all 0.3s; overflow: hidden; }
.kpi-card-sm::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: var(--grad-main); opacity: 0.6; }
.kpi-card-sm:hover { border-color: var(--border); background: var(--primary-soft); transform: translateY(-2px); }
.kpi-sm-label { font-size: 10px; color: var(--text-dim); margin-bottom: 3px; }
.kpi-sm-value { font-size: 16px; font-weight: 700; color: var(--primary); font-family: 'Consolas', monospace; margin-bottom: 4px; }
.kpi-sm-bar { height: 3px; background: rgba(0, 212, 255, 0.1); border-radius: 2px; overflow: hidden; }
.kpi-sm-bar-fill { height: 100%; background: var(--grad-cyan); }

.chart-area { width: 100%; height: 100%; min-height: 140px; position: relative; }
...
.alert-list { display: flex; flex-direction: column; gap: 6px; max-height: 160px; overflow-y: auto; padding-right: 4px; }
.alert-item { display: flex; align-items: center; gap: 8px; padding: 6px 10px; background: linear-gradient(90deg, var(--bg-soft), transparent); border: 1px solid var(--border-soft); border-radius: 6px; transition: all 0.25s; cursor: pointer; position: relative; overflow: hidden; }
...
.alert-item.info::before { background: var(--primary); }
.alert-item.success::before { background: var(--success); }
.alert-item:hover { background: var(--primary-soft); transform: translateX(2px); }

...
const getMapOption = () => {
  ...
  const mapColors = ['#0bc4e9', '#7dd87d', '#f5c542', '#e8554f'];
  ...
  return {
    ...
    visualMap: {
      ...
      max: 100,
      pieces: [
        { min: 0, max: 48, color: mapColors[0] },
        { min: 48, max: 55, color: mapColors[1] },
        { min: 55, max: 60, color: mapColors[2] },
        { min: 60, max: 100, color: mapColors[3] },
      ],
      outOfRange: {
    ...
    geo: {
      ...
      itemStyle: {
        ...
        borderWidth: 1,
        shadowBlur: 40,
        shadowColor: 'rgba(0, 180, 255, 0.45)',
        shadowOffsetY: 8,
      },
      emphasis: {
        label: {
      ...
      select: {
        ...
        itemStyle: { areaColor: 'rgba(11, 196, 233, 0.55)' },
      },
      zlevel: 1,
    },
    series: [
      ...
      },
      {
        ...
        data: [
          ...
          { coords: [[116.4, 39.9], [113.27, 23.13]] },
          { coords: [[116.4, 39.9], [104.06, 30.67]] },
          { coords: [[116.4, 39.9], [108.95, 34.27]] },
          { coords: [[121.47, 31.23], [114.05, 22.55]] },
          { coords: [[121.47, 31.23], [120.15, 30.28]] },
          { coords: [[113.27, 23.13], [114.05, 22.55]] },
          { coords: [[116.4, 39.9], [126.53, 45.8]] },
        ],
      },
      {
        type: 'effectScatter',
...
.map-area {
  ...
  min-height: 0;
  background: 
    radial-gradient(ellipse at 50% 50%, rgba(0, 80, 160, 0.35) 0%, rgba(0, 40, 100, 0.2) 40%, transparent 70%),
    radial-gradient(ellipse at center, rgba(11, 196, 233, 0.12) 0%, rgba(6, 25, 60, 0.25) 50%, transparent 80%),
    linear-gradient(180deg, rgba(5, 18, 45, 0.8) 0%, rgba(3, 12, 30, 0.9) 100%);
  border: 1px solid rgba(0, 229, 255, 0.35);
  border-radius: 4px;
  overflow: hidden;
  box-shadow: 
    0 4px 40px rgba(0, 0, 0, 0.6),
    0 0 40px rgba(11, 196, 233, 0.2),
    inset 0 1px 0 rgba(255, 255, 255, 0.04),
    inset 0 0 60px rgba(0, 100, 180, 0.15);
...
.mc-tl { top: 10px; left: 10px; border-right: none; border-bottom: none; }
.mc-tl::before { top: -3px; left: -3px; }
.mc-tr { top: 10px; right: 10px; border-left: none; border-bottom: none; }
.mc-tr::before { top: -3px; right: -3px; }
.mc-bl { bottom: 10px; left: 10px; border-right: none; border-top: none; }
.mc-bl::before { bottom: -3px; left: -3px; }
.mc-br { bottom: 10px; right: 10px; border-left: none; border-top: none; }
.mc-br::before { bottom: -3px; right: -3px; }

.map-data-strip {
  position: absolute; bottom: 0; left: 230px; right: 0;
  display: flex; justify-content: flex-end; gap: 20px;
  padding: 10px 16px;
  background: linear-gradient(180deg, transparent, rgba(4, 12, 30, 0.96));
  border-top: 1px solid rgba(0, 229, 255, 0.3);
  font-size: 11px; color: rgba(160, 220, 255, 0.85);
  font-family: 'Consolas', 'Monaco', monospace;
  z-index: 5;
}
.map-data-strip b { color: rgba(0, 229, 255, 1); font-weight: 700; text-shadow: 0 0 12px rgba(0, 229, 255, 0.6); }
.map-drill-tip {
  color: #ffffff;
  font-weight: 700;
  text-shadow: 0 0 10px rgba(0, 229, 255, 0.9), 0 0 20px rgba(0, 229, 255, 0.5);
  background: linear-gradient(90deg, rgba(0, 229, 255, 0.25), rgba(0, 140, 215, 0.25));
  border: 1px solid rgba(0, 229, 255, 0.7);
  padding: 4px 12px;
  border-radius: 2px;
  letter-spacing: 1px;
  animation: tip-pulse 2s ease-in-out infinite;
  box-shadow: 0 0 18px rgba(0, 229, 255, 0.45), inset 0 0 12px rgba(0, 229, 255, 0.12);
  cursor: pointer;
}
@keyframes tip-pulse {
  0%, 100% { 
...
.map-legend {
  ...
  z-index: 10;
  padding: 14px 18px;
  background: linear-gradient(180deg, rgba(8, 22, 50, 0.92) 0%, rgba(4, 12, 30, 0.88) 100%);
...
.map-legend::before {
  ...
  box-shadow: 0 0 12px rgba(11, 196, 233, 0.6);

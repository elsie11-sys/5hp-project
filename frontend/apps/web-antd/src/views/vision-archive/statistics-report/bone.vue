  ...
  <div class="report-panel">
    ...
    <div class="report-section">
      <div class="section-header">
        <h3>📋 {{ dimensionLabel }}骨密度偏低率统计报表</h3>
        ...
        <table class="report-table">
          ...
          <tbody>
            <tr v-for="(item, index) in reportData" :key="index">
              ...
              <td>{{ formatNumber(item.lowBone) }}</td>
              <td>
      ...
      <div class="table-footer">
        <div class="summary-info">
          ...
          骨密度偏低 <strong>{{ formatNumber(totalSummary.lowBone) }}</strong> 人
        </div>
        <div class="summary-rate">
          总体骨密度偏低率：<strong>{{ totalSummary.rate }}%</strong>
        </div>
      </div>
    </div>
    ...
    <div class="trend-prediction">
      <div class="prediction-card">
        <div class="prediction-label">当前骨密度偏低率</div>
        <div class="prediction-value">{{ currentRate }}%</div>
      </div>
      <div class="prediction-card">
        <div class="prediction-label">预测下一年度骨密度偏低率</div>
...
const reportData = ref([
  { name: '江苏省', total: 1250000, lowBone: 175000, rate: 14.0, change: 0.5, trend: 'up' },
  { name: '浙江省', total: 980000, lowBone: 127400, rate: 13.0, change: 0.3, trend: 'up' },
  { name: '广东省', total: 1560000, lowBone: 218400, rate: 14.0, change: 0.8, trend: 'up' },
  { name: '山东省', total: 1100000, lowBone: 143000, rate: 13.0, change: -0.2, trend: 'down' },
  { name: '河南省', total: 1080000, lowBone: 140400, rate: 13.0, change: -0.4, trend: 'down' },
  { name: '四川省', total: 950000, lowBone: 123500, rate: 13.0, change: 0.1, trend: 'flat' },
  { name: '湖北省', total: 820000, lowBone: 106600, rate: 13.0, change: 0.0, trend: 'flat' },
  { name: '湖南省', total: 780000, lowBone: 101400, rate: 13.0, change: -0.1, trend: 'down' },
]);

// ================= 汇总数据 =================
const totalSummary = computed(() => {
  const total = reportData.value.reduce((sum, item) => sum + item.total, 0);
  const lowBone = reportData.value.reduce((sum, item) => sum + item.lowBone, 0);
  const rate = total > 0 ? (lowBone / total * 100).toFixed(1) : 0;
  return { total, lowBone, rate };
});

// ================= 趋势预测 =================
const currentRate = ref(13.5);
const predictedRate = ref(14.8);
...
const getRateClass = (rate) => {
  if (rate >= 18) return 'rate-high';
  if (rate >= 12) return 'rate-mid';
  return 'rate-low';
};
...
const generateReport = () => {
  reportData.value = reportData.value.map(item => ({
    ...item,
    rate: (10 + Math.random() * 10).toFixed(1),
  ...
  barChart.setOption({
    tooltip: {
      ...
      formatter: function(params) {
        const p = params[0];
        return `<strong>${p.name}</strong><br/>骨密度偏低率：<strong>${p.value}%</strong>`;
      }
    },
    grid: {
      left: '6%',
    ...
    series: [{
      ...
      itemStyle: {
        ...
        color: function(params) {
          ...
          if (value >= 18) return '#ff4d4f';
          if (value >= 12) return '#faad14';
          return '#52c41a';
        }
      },
      label: {
...
const initTrendChart = () => {
  ...
  const data = [12.5, 12.8, 13.0, 13.2, 13.5, 14.8];
